...
//	std::vector<size_t>
//		sortIndicesDescending(const std::vector<float>& vec)
//	{
...
//	std::vector<size_t>
//		sortIndicesAscending(const std::vector<float>& vec)
//	{
//		std::vector<size_t> idx(vec.size());
//		std::iota(idx.begin(), idx.end(), 0);
//		std::stable_sort(idx.begin(), idx.end(), [&vec](size_t i1, size_t i2) {return vec[i1] < vec[i2]; });
...
//	{
//		std::vector<T_field> vec(indices.size());
...
//	{
//		typename std::vector<T> vec = std::copy<T, T>(begin_it, end_it);
...
//	template <typename T>
//	std::vector<T> getUniqueList(std::vector<T>& vec)
//	{
//		//Get list of classes in the SF
//		std::vector<T> unique_list(vec.size());
...
//			auto end_it = std::find_if(vec.rbegin(), vec.rend(), [&value](T element) {return element == value; });
//
